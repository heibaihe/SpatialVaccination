...
double leak; // the leakiness factor 
...
if((S[k]>1/NK)&&(Ihat[k]>1/NK))  // verify that region k still has infections - otherwise the lockdown factor is 1
    			{
				lk=h/((1-mu)*beta*(S[k]+leak*VS[k])*Ihat[k]);
    				if(lk>1)
    				{
    					lk=1;
				}
...
if((S[k]<1/NK)&&(R[k]<1/NK))  // if there are no susceptible nor recovered:
{
    				Sdot[k]=0;
    				Rdot[k]=r*Ihat[k];
Idot[k]=(1-mu)*lk*beta*Ihat[k]*(S[k]+leak*VS[k])-r*Ihat[k];
    				VSdot[k]=-(1-mu)*lk*beta*Ihat[k]*leak*VS[k];
    				VRdot[k]=0;
			}
...
Sdot[k]=-(1-mu)*lk*beta*Ihat[k]*S[k]-vK[k]*S[k]/(S[k]+R[k]);
    				...
    				VSdot[k]=vK[k]*S[k]/(S[k]+R[k])-(1-mu)*lk*beta*Ihat[k]*leak*VS[k];
...
// arrival rates
			rateMut[k]=dt*mu*lk*beta*Ihat[k]*(S[k]+leak*VS[k])*NK;
...
for(leak=0;leak<0.5;leak=leak+0.01)   // loop for the leakiness factor
{ 
...
fprintf(fp1,"%f  %f\n",leak,count2/(double)Realizations);	
//mu=mu*factor;
}
