...
double hf[K]; // factors controlling the infection rate for each region 
...
{
			// calculate region k¡¯s lockdown factor taking into account the heterogeneous infection factor hf[k] 
if((S[k]>1/NK)&&(Ihat[k]>1/NK))  // verify that region k still has infections - otherwise the lockdown factor is 1
    			{
				lk=hf[k]*h/((1-mu)*beta*S[k]*Ihat[k]);
    				if(lk>1)
    				{
    					lk=1;
				}
...
factor=pow(10,0.1); // step for mu in computation, 0.1 orders of magnitude
    ...
    // set the factors for infection rate when vaccination from regions with low infection rate
    hf[0]=1;
	hf[1]=2;
	hf[2]=3;
	hf[3]=4;
    ...
    // set the factors for infection rate when vaccination from regions with high infection rate
  /*hf[0]=10;
	hf[1]=9;
	hf[2]=8;
	hf[3]=7;
