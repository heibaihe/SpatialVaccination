...
double TralLmit; //Travel limitation factor 
...
if (K>1)
{
				...
				for(i=0;i<K;i++)
				{
					...
					{
						TralLmit=1;
						if((control>0)&&(((i<kk)&&(k>kk))||((i>kk)&&(k<=kk)))) // the contact rate between vaccinated and unvaccinated regions is half
						{
							TralLmit=0.5;
						}
						Ihat[k]=Ihat[k]+TralLmit*c*I[i]/(K-1);
						Imhat[k]=Imhat[k]+TralLmit*c*Im[i]/(K-1);
					}
				}
			}
