...
double S[100],Sdot[100],I[100],Idot[100],R[100],Rdot[100],VS[100],VSdot[100],VR[100],VRdot[100],Im[100],Ihat[100],Imhat[100],sumS,sumI,sumR,sumVS,sumVR,sumM;
...
double rateMut[100],rateInf[100],rateRec[100],nMut[100],nInf[100],nRec[100];

double t;  // time (computed in loop)
double control; // vaccination phase (=-1 before vaccination starts, =1 during vaccination phase) 
double vK[100]; // vaccination rate in each region

int K; // number of regions
int KK[19]; // array storing number of regions for the loop of K
...
int main(){
	...
	mu=pow(10,-8); // initial mutation rate 10^(-10.2)
...
for(jj=0;jj<19;jj++)   // loop for the mutation rate, from 10^(-10.2) to 10^(6.8) step 10^(0.1)
{ 
    K=KK[jj];
...
fprintf(fp1,"%d  %f\n",KK[jj],count2/(double)Realizations);	
mu=mu*factor;
}
