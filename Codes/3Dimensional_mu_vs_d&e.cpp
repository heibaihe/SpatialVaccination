...
double d; // fitness deficiency for mutants
double e;  // escape capacity for mutants

double mu1,mu2,muh,mul,count3; // variables for search mu(P=50%) by bisection
...
// arrival rates
			rateMut[k]=dt*mu*lk*beta*Ihat[k]*S[k]*NK;
    			rateInf[k]=dt*lk*betam*(e*VS[k]+S[k])*Imhat[k];
...
int main(){
	...
	mul=1*pow(10,-11); // maximum mutation rate for test
	muh=1*pow(10,-5); // minimum mutation rate for test
factor=pow(10,0.04); // step for mu in computation, 0.1 orders of magnitude

	srand(time(NULL)); // seed for random number 
    

// loops for deficiency d and evasion e    
for(e=0.4;e<1.01;e=e+0.01) 
{ 
	for(d=0.4;d<1.21;d=d+0.01) 
    { 
    betam=d*beta; // infection rate for mutants
    count3=0;
    // initiate the testing mutation rate 
    mu1=mul;
    mu2=muh;
    mu=mu1;
	...
	while(fabs(count3-0.5)>0.005)
   	{ 			
		...
		count3=count2/(double)Realizations;
		
		// if the probability is not 0.5, then change mu using bisection
		if(count3>0.5005)
   		{   
   		    mu2=mu;
   			mu=(mu+mu1)/2;
...
fprintf(fp1,"%.12f  %f  %f  %f\n",mu,d,e,count3/(double)Realizations);	
}
//mu=mu*factor;
}
