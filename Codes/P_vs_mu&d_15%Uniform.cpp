...
for(t=0;t<T;t=t+dt)
	{
		// test if we are already in the vaccination phase (second year) and start the 15% homogeneous vaccination 
		...
		// start spatial vaccination after the 15% homogeneous vaccination 
		if((t>(365+68)/4.0-pow(10,-8))&&(t<(365+68)/4.0+pow(10,-8)))
		{
			for(k=0;k<K;k++)
			{
				vK[k]=0;
...
// arrival rates
			rateMut[k]=dt*mu*lk*beta*Ihat[k]*S[k]*NK;
    			rateInf[k]=dt*lk*betam*(VS[k]+S[k])*Imhat[k];
...
for(jj=0;jj<51;jj++)   // loop for the mutation rate, from 10^(-10.2) to 10^(6.8) step 10^(0.1)
{ 
for(d=0.4;d<1.21;d=d+0.01) // loop for the deficiency d 
    {
    betam=d*beta; // infection rate for mutants
...
fprintf(fp1,"%.12f  %f  %f\n",mu,d,count2/(double)Realizations);	
}
mu=mu*factor;
}
